...
namespace CISSAPortal.Controllers
{
    ...
    public class ReportsController : Controller
    {
        ...
        public ActionResult SelectHumDistributionPlan()
        {
            ...
            var plans = humDistributionPlans.ToList();
            if (plans.Count > 0)
                return View(plans);
            else throw new Exception("Принятые планы отсутствуют!");
        }
        ...
        public ActionResult CreateReport(Report report)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    if (db.Reports.Any(x => x.UserId == report.UserId && x.Year == report.Year && x.Quarter == report.Quarter && x.State.Code != 5))
